...
class ModelArguments:
    ...
    freeze_embed_prompts: bool = field(
        default=False,
        metadata={"help": "Whether to freeze the embed_prompts."},
    )
    freeze_cross: bool = field(
        default=False,
        metadata={"help": "Whether to freeze the cross modules."},
    )
    ...
    asr_model_name_or_path: str = field(
        default="distil-whisper/distil-large-v3", #distil-whisper/distil-large-v2
        metadata={
    ...
    prompt_padding_side: Optional[str] = field(
        default="right",
        metadata={
            "help": "Prompt tokenizer padding side. Defaults to `right`. If the prompt is pre-pended to the codebooks hidden states, it should be padded on the left."
        },
    )
    rope_embeddings: bool = field(
        default=False,
        metadata={"help": "Whether to use ROPE or absolute positional embeddings."},
    )
    track_pattern: Optional[str] = field(
        default='mixed_pro',
        metadata={
            "help": "The token pattern used. Mixed Mode: ['mixed', 'mixed_pro']; Dual-track Mode: ['parallel_std', 'parallel_A_V', 'parallel_V_A', 'interleaving_A_V', 'interleaving_V_A']"
        },
    )
    prompt_cross_attention: bool = field(
        default=True,
        metadata={"help": "Whether to use cross-attention conditioning for the lyrcics (as well as the description)"},
    )
    add_prenet: bool = field(
        default=True,
        metadata={"help": "Whether to use lyrics encoder."},
    )
...
class DataTrainingArguments:
    ...
    ref_audio_column_name: Union[List[str], str] = field(
        default=None,
        metadata={"help": "The name of the dataset column containing the ref audio data. Defaults to 'None'. eg., melody, drum, acc, vocal, random"},
    )
    ref_voice_column_name: str = field(
        default=None,
        metadata={"help": "The name of the dataset column containing the reference voice data. Defaults to 'None'."},
    )
    ref_dur_sec: float = field(
        default=3,
        metadata={ "help": "the duration of reference voice data. Defaults to 3." },
    )
    ...
    loading_method: str = field(default='hf', metadata={"help": "method of loading: json, disk or hf"})
    label_atype: str = field(default='labels', metadata={"help": "the target audio type: vocal, acc, mix(default labels)"})


@dataclass
class SongGenTrainingArguments(Seq2SeqTrainingArguments):
    ...
    codebook_weighting: bool = field(
        default=True,
        metadata={"help": ("Whether to apply weighted summation of losses from multiple codebooks.")},
    )
    add_vocal_loss: bool = field(
        default=False,
        metadata={"help": ("Whether to include an auxiliary vocal token prediction target.")},
    )
