...
def main():
    ...
    ####### A. Preparation
    kwargs_handlers = [InitProcessGroupKwargs(timeout=timedelta(minutes=120))] #NOTE 
    ...
    data_collator = DataCollatorSongGenWithPadding( 
        ...
        energy_name='_clip_energy',  # or "_energy"
    ...
    def train_step(
        ...
        if outputs.get("codebook_losses", None) is not None:
            ...
            for i, loss in enumerate(outputs.codebook_losses): 
                ...
                if weight_loss:
                    total_codebook_loss += codebook_weights[i] * loss
                    metrics[f'loss_weight_{i}'] = codebook_weights[i] 
            if weight_loss:
    ...
    def decode_codes_audios(labels, accelerator):
        eval_model = accelerator.unwrap_model(model, keep_fp32_wrapper=True)
        if training_args.torch_compile:
        ...
        for label in labels:
            ...
            if not mask.any(): 
                mask_random = (label==1030) 
                label = label[mask_random]
                label = label.reshape(1, 1, 8, -1)
                audio_len= int(label.shape[-1] / 50 * 16000)
                audio = torch.zeros(audio_len, 1, 1, device=labels.device)
                label_audios.append(audio)
                output_lengths.append(audio_len)
                continue
            ...
            with torch.no_grad():
                audio = audio_decoder.decode(label, [None]).audio_values  
                    ...
                    if training_args.predict_with_generate:
                        ...
                        for batch in tqdm(
                            ...
                            if log_gt_first_flag:
                                ...
                                label_audios_dict = decode_codes_audios(batch['labels'], accelerator)
                                ...
                                if "ref_audio_ids" in batch:
                                    ref_audios_dict = decode_codes_audios(batch['ref_audio_ids'], accelerator)
