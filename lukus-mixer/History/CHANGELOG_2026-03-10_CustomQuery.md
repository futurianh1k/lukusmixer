...
**v1.1 업데이트**: 기존 모델 분리와 커스텀 쿼리 분리를 **동시에** 실행할 수 있도록 통합했습니다.
...
### 2. 통합 분리 (v1.1)
1. 모델 선택 (예: 6스템, 10스템 등)
2. "Custom Query Mode" 토글 활성화
3. 분리할 커스텀 쿼리 선택 (체크박스)
4. **"Split Stems + N개 커스텀"** 버튼 클릭
5. 결과: 모델 스템 + 커스텀 스템이 함께 표시됨
...
### Backend 변경
- `SplitRequest`에 `custom_query_ids` 옵션 파라미터 추가
- `process_split_job()`: 모델 분리 → 커스텀 쿼리 분리 순차 실행
- 진행률 할당: 모델 70%, 커스텀 쿼리 20%, 스펙트로그램 10%
- 결과에 `is_custom` 플래그로 커스텀 스템 구분

### Frontend 변경
- `handleSplit()`: 항상 `/api/split` 호출, 커스텀 쿼리 선택 시 `custom_query_ids` 포함
- 버튼 텍스트: "Split Stems + N개 커스텀"
- 토스트 메시지에 모델/커스텀 개수 표시
